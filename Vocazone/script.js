...
function openDetailModal(id) {
    ...
    // Reset Registration Button & Form State
    ...
    if (regBtn) {
        ...
        regBtn.classList.remove('opacity-75', 'cursor-not-allowed', 'bg-green-500', 'hover:bg-green-600');
        regBtn.classList.add('bg-polihasnur-navy', 'hover:bg-blue-900');
        if (regBtnText) regBtnText.innerText = 'Daftar Sekarang';
    }
...
function closeDetailModal() {
    ...
    // Reset registration form state if it exists
    const regForm = document.getElementById('modal-registration-form');
    ...
    const inputs = ['reg-name', 'reg-nim', 'reg-team'];
    inputs.forEach(id => {
...
function handleRegistration() {
    ...
    // State 2: Validate & Submit (Simulated)
    const name = document.getElementById('reg-name').value;
    const nim = document.getElementById('reg-nim').value;

    if (!name || !nim) {
        showToast('Validasi Gagal', 'Mohon isi Nama dan NIM Anda.', 'error');
        return;
    }
    ...
    setTimeout(() => {
        // Success
        btn.innerHTML = '<i class="fa-solid fa-check"></i> Pendaftaran Terkirim!';
        btn.classList.remove('bg-polihasnur-navy', 'hover:bg-blue-900');
        btn.classList.add('bg-green-500', 'hover:bg-green-600');

        // Show Modern Toast
        showToast('Berhasil!', `Selamat ${name}! Pendaftaran Anda telah kami terima.`, 'success');
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                btn.classList.add('bg-polihasnur-navy', 'hover:bg-blue-900');
                // Clear inputs
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Attach Registration Event Listener
    ...
    btn.addEventListener('click', () => {
        ...
        document.querySelectorAll('.faq-btn').forEach(otherBtn => {
            if (otherBtn !== btn) {
                ...
                otherBtn.nextElementSibling.classList.remove('opacity-100');
            }
        });
...
if (newsletterForm) {
    newsletterForm.addEventListener('submit', (e) => {
        ...
        if (emailInput.value) {
            // Simulate API call
            const originalBtnContent = newsletterForm.querySelector('button').innerHTML;
            newsletterForm.querySelector('button').innerHTML = '<i class="fa-solid fa-check"></i>';
            emailInput.value = 'Terima kasih telah berlangganan!';
            emailInput.setAttribute('disabled', 'true');

            setTimeout(() => {
                newsletterForm.querySelector('button').innerHTML = originalBtnContent;
                emailInput.value = '';
...
function showToast(title, message, type = 'info') {
    ...
    toast.innerHTML = `
        ...
        <div class="toast-progress" style="color: var(--toast-color)"></div>
    `;
